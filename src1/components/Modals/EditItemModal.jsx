...
const customStyles = {
  content: {
    ...
    borderRadius: "10px",
    zIndex: 10000,
    padding: "0", // Remove default padding to control it manually
  },
  overlay: {
...
const EditItemModal = ({ isOpen, item, onClose, onItemUpdated }) => {
 // console.log("EditItemModal - Component rendered with props:", { isOpen, item, onItemUpdated: !!onItemUpdated, onClose: !!onClose });
  ...
  useEffect(() => {
    ...
    if (item) {
      console.log("EditItemModal - Setting form data with item:", {
        iname: item.iname,
        unit: item.unit,
        weight: item.weight,
        tax: item.tax,
      ...
      console.log("EditItemModal - No item received");
    }
  }, [item]);
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        // Use fetchData for taxes, fetchComboData for units and categories  
        const [taxes, units, categories] = await Promise.all([
          fetchData("taxes", null, "id", {}),
          fetchComboData("units", "name"), // Use fetchComboData for units
          fetchComboData("categories", "name"), // Use fetchComboData for better category display
        ]);
        ...
        if (item && item.catid) {
         // console.log("Item has category, fetching subcategories for category:", item.catid);
          try {
            const subCats = await fetchComboDataWithWhere("subcategory", "subcat", {
              cat_id: item.catid
            });
          //  console.log("Fetched subcategories for item category:", subCats);
