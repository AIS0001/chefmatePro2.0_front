...
const EditItemModal = ({ isOpen, item, onClose, onItemUpdated }) => {
  ...
  // Static quantity types (as per UI)
  const quantityTypes = [
    { id: 1, value: "Per Piece", label: "Per Piece" },
    { id: 2, value: "Per KG", label: "Per KG" },
  ];
  ...
  useEffect(() => {
    if (item && isOpen) {
      ...
      const inferredQuantityType = item.quantity_type || item.weight || (
        unitValue === "kg" || unitValue === "kgs" ? "Per KG" :
        unitValue === "pcs" || unitValue === "piece" || unitValue === "pieces" ? "Per Piece" :
        ""
      );
      ...
      form.setFieldsValue({
        ...
        iname: item.iname || "",
        unit: item.unit || "",
        quantity_type: inferredQuantityType,
        tax: item.tax || "",
        mrp: item.mrp || "",
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      ...
      try {
        // Fetch taxes, units, and categories
              ...
              >
                <Select placeholder="Select type" allowClear>
                  {quantityTypes.map((qt) => (
                    <Select.Option key={qt.id} value={qt.value}>
                      {qt.label}
                    </Select.Option>
                  ))}
                </Select>
