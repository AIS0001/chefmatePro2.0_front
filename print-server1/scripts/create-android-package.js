...
function getAvailablePrinters() {
  try {
    if (os.platform() === 'android' || process.env.TERMUX_VERSION) {
      ...
      return [
        { name: 'DEFAULT_PRINTER', status: 'Available' }
      ];
    }
  } catch (error) {
    ...
    try {
      ...
      if (os.platform() === 'android' || process.env.TERMUX_VERSION) {
        ...
        exec(printCommand, (error, stdout, stderr) => {
          ...
          if (error) {
            console.log('USB print failed, trying alternative method...');
            // Alternative: save to downloads folder for manual printing
...
app.listen(PORT, () => {
  ...
  const termuxInstallScript = `#!/bin/bash
echo "ChefMate Print Server - Android/Termux Setup"
echo "============================================"
echo
...
`;
  ...
  fs.writeFileSync(path.join(androidDir, 'install.sh'), termuxInstallScript);
  
  // Create start script
  const startScript = `#!/bin/bash
echo "ChefMate Print Server - Starting..."
echo "=================================="
echo
...
export PRINTER_CONFIG_KITCHEN="USB_PRINTER"
export PRINTER_CONFIG_CASHIER="USB_PRINTER"
...
node server-android.js`;
  ...
  const androidReadme = `# ChefMate Print Server - Android Version

## 📱 Installation on Android (Termux)

1. Install Termux from F-Droid or Google Play
2. Copy this folder to your Android device
...
- Bluetooth printers (via USB bridge)
- File-based printing (saves to Downloads)
...
Edit \`server-android.js\` to change printer settings.
`;

  fs.writeFileSync(path.join(androidDir, 'README.md'), androidReadme);
  ...
  // Create ZIP archive
  console.log('📦 Creating Android ZIP archive...');
