            ...
            <CardComponent>
              <div className='d-flex flex-wrap gap-2 justify-content-end'>
                ...
                <button
                  ...
                  onClick={async () => {
                    // Always fetch latest bill from DB for print preview
                    let latestBillId = window.lastSavedBillId;
                    if (!latestBillId) {
                      // Try to fetch latest bill from DB if not set
                      try {
                        const res = await axios.get('/getlatestbill');
                        latestBillId = res.data && res.data.bill_id;
                        if (latestBillId) window.lastSavedBillId = latestBillId;
                      } catch (err) {
                        toast.error('Could not fetch latest bill for print preview.');
                        return;
                      }
                    }
                    if (!latestBillId) {
                    ...
                    // Show preview (GSTInvoicePrintPreview will fetch DB data via getGSTInvoiceProps)
                ...
                <button
                  ...
                  style={{ backgroundColor: '#E442E4FF' }}
                  onClick={async () => {
                    // Always fetch latest bill from DB for VAT print preview
                    let latestBillId = window.lastSavedBillId;
                    if (!latestBillId) {
                      try {
                        const res = await axios.get('/getlatestbill');
                        ...
                        toast.error('Could not fetch latest bill for VAT print preview.');
                        return;
                      }
                    }
                    if (!latestBillId) {
